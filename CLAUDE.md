...
- `Form\Extension\ProductTypeExtension` adds the `priceTiers` field to the Sylius `ProductType` and forwards the parent product down as `entry_options.product` so each per-row `PriceTierType` (including freshly-added rows that aren't yet linked to a product) can render the variant selector for the current product. `Form\Type\PriceTierCollectionType` extends `Symfony\UX\LiveComponent\Form\Type\LiveCollectionType` (mirroring how Sylius admin handles product images) so add/delete fire server-side via Symfony UX Live Components — no client-side prototype-cloning JS. It uses an `entry_options` normalizer to always merge `label => false` with whatever the caller passes (so the product passed down by `ProductTypeExtension` doesn't wipe the label default). To survive LiveCollectionType's empty-bind cycle on `addCollectionItem`, `PriceTier::setQuantity()` and `setDiscount()` accept null and reset the property to the corresponding class default (`DEFAULT_QUANTITY = 1`, `DEFAULT_DISCOUNT = '0.0'`) — keeps the workaround out of `PriceTierType` and makes the model the single source of truth for its defaults.
- `Form\Type\ProductVariantAutocompleteType` is a Symfony UX autocomplete (`#[AsEntityAutocompleteField(alias: 'setono_sylius_tier_pricing_product_variant', route: 'sylius_admin_entity_autocomplete')]`) that extends Sylius admin's `TranslatableAutocompleteType`. `PriceTierType` passes the parent product's id through `extra_options.product_id`; the autocomplete's `filter_query` default reads that back at request time and adds `entity.product = :id` to the QueryBuilder, so the dropdown only ever returns the current product's variants. Don't pass the `ProductInterface` object itself through `extra_options` — Symfony UX validates that every value is scalar/array/null (`AutocompleteChoiceTypeExtension::validateExtraOptions`), since `extra_options` is base64-encoded into the autocomplete URL and round-tripped through the server.
...
- Required scalar setters on entities that participate in a `LiveCollectionType` should accept null and reset the property to its class default, instead of forcing `empty_data` defaults at the form level. LiveCollectionType re-binds the parent form on `addCollectionItem` before the user types anything; non-nullable setters explode with `InvalidTypeException`, and pushing the default to the form means the model has two sources of truth for its initial state. Reset-on-null beats no-op-on-null because the post-rebind state is deterministic (`quantity` is always the default after a null bind, regardless of any previous value).
...
- **Every new feature, service, class, form type, twig hook, or convention shift must land with a test in the same change.** Unit tests live in `tests/<topic>/` and run under PHPUnit; functional tests live in `tests/Functional/` and boot the embedded Sylius test app via `tests/Application/Kernel.php`. Pick whichever surface gives the strongest signal — pure logic and option-resolver wiring is faster as a unit test, while anything touching the container, the database, or rendered Twig hooks belongs in a functional test. If the work has both a unit and an integration surface (e.g. a new form type *and* an admin-page hook), cover both. Mutation testing (`vendor/bin/infection`) enforces the floor — adding code without tests will drop MSI below the configured minimum and break CI.
